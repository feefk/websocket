...
#include "base/message_pump_libevent.h"
    ...
    #endif
#elif !defined(BUILD_IN_CHROME)
    #include "base/message_pump_dispatcher.h"
    #include "base/message_pump_observer.h"
    #include "base/message_pump_proxy.h"
    namespace base{
        typedef MessagePump MessagePumpForUI;
    };
#elif !defined(OS_MACOSX) && !defined(OS_ANDROID) && !defined(USE_X11)
    #if defined(USE_AURA)
...
class BASE_EXPORT MessageLoopNestable : public MessageLoop {
 public:
    MessageLoopNestable() : MessageLoop(TYPE_NESTABLE) { }
    // Returns the MessageLoopForIO of the current thread.
    static MessageLoopNestable* current() {
    MessageLoop* loop = MessageLoop::current();
    DCHECK_EQ(MessageLoop::TYPE_NESTABLE, loop->type());
    return static_cast<MessageLoopNestable*>(loop);
    }

    void RegisterStub(base::MessagePumpStub* stub)
    {
        pump_proxy()->Attach(this);
        pump_proxy()->RegisterStub(stub);
    }

 private:
   base::MessagePumpProxy* pump_proxy() {
     return static_cast<base::MessagePumpProxy*>(pump_.get());
   };
};
// Do not add any member variables to MessageLoopForIO!  This is important b/c
// MessageLoopForIO is often allocated via MessageLoop(TYPE_IO).  Any extra
// data that you need should be stored on the MessageLoop's pump_ instance.
COMPILE_ASSERT(sizeof(MessageLoop) == sizeof(MessageLoopNestable),
                MessageLoopNestable_should_not_have_extra_member_variables);
#endif
